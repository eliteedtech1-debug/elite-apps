...
BEGIN
    ...
    ELSEIF query_type = 'select-all' THEN
    	IF p_created_by=1 THEN
        	SELECT * FROM school_setup;
        ELSE
        	 SELECT * FROM school_setup WHERE created_by=p_created_by;
        END IF;
...
ALTER TABLE `school_section_table` ADD `status` ENUM('Active','Inactive') NOT NULL DEFAULT 'Active' AFTER `section_address`;