/**
 * Student Charges API - No Authentication Required
 * Specifically for EditBillModals custom items functionality
 */

const models = require('../models');

module.exports = (app) => {
  // Create student charges endpoint (no auth required)
  app.post('/api/accounting/transactions/create-student-charges', async (req, res) => {
    try {
      console.log('🔧 Creating student charges (no auth):', {
  ...
  // FIXED: Enhanced endpoint for direct payment entry creation (fallback for EditBillModals)
  app.post('/api/accounting/transactions/create-payment-entry-direct', async (req, res) => {
    try {
      console.log('🔧 Direct payment entry creation (ORM-based):', req.body);
      ...
      if (!admission_no || (!amount && !netAmount) || !description) {
        return res.status(400).json({
          success: false,
          message: 'Missing required fields: admission_no, amount/netAmount, description'
        });
      }

      const finalAmount = netAmount || (parseFloat(amount) * parseInt(quantity));
      
      // Generate reference number
      const ref_no = `REF-${Date.now()}-${Math.floor(Math.random() * 1000)}`;

      // Use direct SQL for ORM-based insertion
      const insertSQL = `
      ...
      const [insertResult] = await models.sequelize.query(insertSQL, {
      ...
      console.log('✅ Payment entry created with ID:', newItemId);
      ...
      res.status(201).json({
        ...
        system: 'DIRECT_SQL_ORM'
      });

    } catch (error) {
      console.error('❌ Error creating direct payment entry:', error);
  ...
  app.get('/api/accounting/transactions/health', (req, res) => {
    res.json({
      success: true,
      message: 'Student charges API is healthy',
      endpoints: {
        'POST /api/accounting/transactions/create-student-charges': 'Create student charges with accounting entries (no auth)',
        'POST /api/accounting/transactions/create-payment-entry-direct': 'Create payment entry directly using ORM (no auth)'
      },
      timestamp: new Date().toISOString()
    });
  });
