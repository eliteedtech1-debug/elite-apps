-- =====================================================
-- ADMISSION MODULE - PRODUCTION MIGRATION SCRIPT (FIXED)
-- =====================================================
-- Version: 1.1
-- Date: 2025-12-13
-- Description: Complete migration for admission module with token system
-- Fix: Removed problematic audit log insert at end
-- =====================================================
...
SET @sql = (SELECT IF(
  ...
  'SELECT "Index idx_school_applicants_school_branch already exists" as message'
));
...
SET @sql = (SELECT IF(
  ...
  'SELECT "Index idx_school_applicants_status already exists" as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = (SELECT IF(
  ...
  'SELECT "Index idx_school_applicants_academic_year already exists" as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = (SELECT IF(
  ...
  'SELECT "Column admission_token_used already exists" as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = (SELECT IF(
  ...
  'SELECT "Column access_method already exists" as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
