...
### 1. **ELITE SCHOLAR** (Your Primary Brand)
...
const getEnvironmentVariables = (): EnvironmentVariables => {
  return {
    ...
    appName: env.VITE_APP_NAME || env.REACT_APP_NAME || env.APP_NAME || 'ELITE SCHOLAR',
    // ...
  };
};
```
...
✅ ELITE SCHOLAR: mini-logo.png (Square, Green theme)
...
### Scenario 1: Developer Runs Elite Scholar
...
# → Shows "ELITE SCHOLAR Education Platform"
# → Theme: Green
# → Logo: mini-logo.png
```
...
./scripts/verify-brand-identity.sh
# ❌ ERROR: Wrong logo for ELITE SCHOLAR!
...
|-------|--------|---------|
| ✅ Elite Scholar .env | PROTECTED | Star warnings + comments added |
| ✅ Skcooly Plus .env | PROTECTED | Star warnings + comments added |
| ✅ Elite Scholar Logo File | EXISTS | `/assets/img/mini-logo.png` (18KB) |
...
1. **NEVER modify these lines in .env:**
   ```bash
   VITE_APP_LOGO=/assets/img/mini-logo.png  # ELITE SCHOLAR
...
1. **Your Brand is Sacred**
   - `mini-logo.png` = Elite Scholar loader logo
...
3. **Automatic Domain Detection**
   - `elitescholar.ng` → Elite Scholar brand
   - `skcoolyplus.org.ng` → Skcooly brand
   - `localhost` → Elite Scholar (default)
...
> "My business brand identity (ELITE SCHOLAR with mini-logo.png)
...
|-------|------------------|-------------|--------|
| **ELITE SCHOLAR** | `/assets/img/mini-logo.png` | `.env` | `elitescholar.ng` |
