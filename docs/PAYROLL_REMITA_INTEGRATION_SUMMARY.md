...
The Elite Core system now supports **generic payment gateway integration** for payroll salary disbursement, with **Remita** as the first implemented gateway. The system is designed to support multiple payment gateways (Paystack, Flutterwave, Stripe, etc.) without schema changes.
