# AI Agent Configuration for Elite Core
...
## 🗄️ Direct Remote Database Access

The production database is directly accessible. Use this for running migrations, backfills, and quick queries without SSH.
...
### Run a Migration
```bash
mysql -h 62.72.0.209 -P 3306 -u kirmaskngov_skcooly -pSkoooly2025 kirmaskngov_skcooly_db -e "YOUR SQL HERE;"
```

### Run a SQL File
```bash
mysql -h 62.72.0.209 -P 3306 -u kirmaskngov_skcooly -pSkoooly2025 kirmaskngov_skcooly_db < elscholar-api/src/migrations/your_migration.sql
```

### Verify a Migration
```bash
mysql -h 62.72.0.209 -P 3306 -u kirmaskngov_skcooly -pSkoooly2025 kirmaskngov_skcooly_db -e "DESCRIBE classes;"
```

### Agent Rule
> When a migration SQL file is created, **always offer to run it directly on the remote DB** using the credentials above. No manual steps needed.
